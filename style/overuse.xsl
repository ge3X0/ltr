...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="//pause_abs1">Aufgrund der Intensität der Schmerzen, insbesondere der Migräne, werden die Grenzschwellen für die Entstehung von Kopfschmerzen bei Medikamentenübergebrauch eindeutig überschritten.</xsl:template>
    <xsl:template match="//pause_abs2">Es besteht eine Aufrechterhaltung und weitere Verstärkung des sowieso bestehenden Kopfschmerzes bei Medikamentenübergebrauch.</xsl:template>
    <xsl:template match="//pause_entzug">sowie der Entzugsbehandlung</xsl:template>
    <xsl:template match="//pause_dauer">und der notwendigen Minimierung des Rückfallrisikos in den Analgetika-Übergebrauch und Analgetika-Fehlgebrauch </xsl:template>
    <xsl:template match="//pause_rebound">, schwere Rebound-Kopfschmerzen mussten mit Escape-Medikation behandelt werden, welche die multimodalen Behandlungsmaßnahmen behinderten</xsl:template>
