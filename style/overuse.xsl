...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="//pause_empfehlung"><xsl:if test="$diag_overuse">Des Weiteren empfehlen wir die ambulante Fortführung der Analgetikapause für insgesamt vier Wochen. In dieser Phase kann es sinnvoll sein <xsl:call-template name="patient_akk"/> arbeitsunfähig zu schreiben.</xsl:if></xsl:template>
        ...
        <xsl:if test="$diag_overuse">
            ...
            <w:p>
                ...
                <xsl:call-template name="text-run">
                    <xsl:with-param name="highlight" select="true()"/>
                    <xsl:with-param name="text">
			<xsl:text> für jegliche Kopfschmerzakutmedikation durch. Dabei wurde auf eine intravenöse und orale Gabe von Prednisolon nach einem festen Zeitschema verzichtet. Bedarfsweise erhielt </xsl:text>
			<xsl:call-template name="patient"/>
			<xsl:text> Medikamente zur Schmerzdistanzierung.</xsl:text>
                    </xsl:with-param>
                </xsl:call-template>
            </w:p>
        </xsl:if>
